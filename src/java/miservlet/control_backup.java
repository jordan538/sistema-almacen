...
public class control_backup extends HttpServlet {
    ...
    /*protected void ConsulProducto(HttpServletRequest request, HttpServletResponse response) 
        ...
        String pag="/modDemo.jsp";
        request.getRequestDispatcher(pag).forward(request, response);
        response.getOutputStream().print("<script>window.open='control?opc=2&cod=cod_prod';</script>");     
    }*/
    //AJAX
    protected void ConsulProducto(HttpServletRequest request, HttpServletResponse response) 
        throws ServletException, IOException {
        ...
        //Categorias categorias = objcat.lisCategoria();
        PrintWriter out = response.getWriter();
        Gson gson = new Gson();
